(* Test Plan:
   For our testing, we plan to test everything via OUnit except for 
   main.ml (the interactivity aspect). We will test main ml manually,
   and achieve coverage on it by using every command with multiple
   different outputs. The OUnit tests for all of the modules will start
   as glass-box focused, but we will end with more black-box cases for 
   module 4 (Buy/Sell trigger functionality) since that has a client side
   aspect to it. This testing approach demonstrates the correctness of the
   system because it tests the client-side functionality as if a client were
   actually using the software. Since the rest is backend, we test it using 
   glass-box principles because we want consistent inputs and outputs.
   *)

(* [LOC.txt] Disclaimer to Kabir(PG): In the unit there are a few duplicate 
   test cases (<10) that we left in the test suite because it doesn't
   affect the testing, LOC, or amount of test requirments. We have 
   2.3k LOC, so even without these duplicates we are well over the LOC
   expectation. We also have over 100 test cases, so we are well over 
   the 50 required cases. *)
open OUnit2
